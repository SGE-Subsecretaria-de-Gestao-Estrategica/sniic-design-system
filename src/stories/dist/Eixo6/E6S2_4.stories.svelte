...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    // 2. Establish uniform color rank based on "EC"
    const ecItems =
      grouped
        .get("EC")
        ?.slice()
        .sort((a, b) => b.pct.value - a.pct.value) ?? [];
    const colorRank = new Map(ecItems.map((d, i) => [d.skinColor, i]));

    // 3. PASS 1: Calculate raw vertical layouts & record max 'y' per skinColor
    ...
    for (const [scope, items] of grouped) {
      const sorted = items
        ...
        .sort(
          (a, b) =>
            (colorRank.get(a.skinColor) ?? 0) -
            (colorRank.get(b.skinColor) ?? 0),
        );
