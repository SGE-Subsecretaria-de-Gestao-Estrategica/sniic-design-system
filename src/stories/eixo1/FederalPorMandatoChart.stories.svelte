<script module lang="ts">
  ...
  import FederalPorMandatoChart from '$lib/components/eixo1/FederalPorMandatoChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Orçamento/Federal/Gasto por mandato presidencial',
    component: FederalPorMandatoChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof FederalPorMandatoChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <FederalPorMandatoChart {...args} />
  </StoryFrame>
{/snippet}
