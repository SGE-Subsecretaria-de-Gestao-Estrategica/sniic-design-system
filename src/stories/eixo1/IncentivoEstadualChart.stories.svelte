<script module lang="ts">
  ...
  import IncentivoEstadualChart from '$lib/components/eixo1/IncentivoEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Lei de incentivo',
  ...
  type StoryArgs = ComponentProps<typeof IncentivoEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <IncentivoEstadualChart {...args} />
  </StoryFrame>
{/snippet}
