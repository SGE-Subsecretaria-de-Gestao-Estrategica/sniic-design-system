<script module lang="ts">
  ...
  import ParidadeConselhosEstadualChart from '$lib/components/eixo1/ParidadeConselhosEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Paridade dos conselhos',
    component: ParidadeConselhosEstadualChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof ParidadeConselhosEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <ParidadeConselhosEstadualChart {...args} />
  </StoryFrame>
{/snippet}
