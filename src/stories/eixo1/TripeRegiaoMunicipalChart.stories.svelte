<script module lang="ts">
  ...
  import TripeRegiaoMunicipalChart from '$lib/components/eixo1/TripeRegiaoMunicipalChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão municipal/Tripé por região',
    component: TripeRegiaoMunicipalChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof TripeRegiaoMunicipalChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <TripeRegiaoMunicipalChart {...args} />
  </StoryFrame>
{/snippet}
