<script module lang="ts">
  ...
  import TransversalidadeEstadualChart from '$lib/components/eixo1/TransversalidadeEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Transversalidade',
    component: TransversalidadeEstadualChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof TransversalidadeEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <TransversalidadeEstadualChart {...args} />
  </StoryFrame>
{/snippet}
