<script module lang="ts">
  ...
  import BibliotecaMunicipalChart from '$lib/components/eixo1/BibliotecaMunicipalChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão municipal/Biblioteca pública',
    component: BibliotecaMunicipalChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof BibliotecaMunicipalChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <BibliotecaMunicipalChart {...args} />
  </StoryFrame>
{/snippet}
