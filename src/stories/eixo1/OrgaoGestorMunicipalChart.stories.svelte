<script module lang="ts">
  ...
  import OrgaoGestorMunicipalChart from '$lib/components/eixo1/OrgaoGestorMunicipalChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão municipal/Órgão gestor',
    component: OrgaoGestorMunicipalChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof OrgaoGestorMunicipalChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <OrgaoGestorMunicipalChart {...args} />
  </StoryFrame>
{/snippet}
