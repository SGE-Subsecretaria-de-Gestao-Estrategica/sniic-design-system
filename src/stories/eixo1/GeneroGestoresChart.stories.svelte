<script module lang="ts">
  ...
  import GeneroGestoresChart from '$lib/components/eixo1/GeneroGestoresChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão municipal/Sexo dos gestores',
  ...
  type StoryArgs = ComponentProps<typeof GeneroGestoresChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <GeneroGestoresChart {...args} />
  </StoryFrame>
{/snippet}
