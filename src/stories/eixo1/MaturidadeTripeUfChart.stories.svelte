<script module lang="ts">
  ...
  import MaturidadeTripeUfChart from '$lib/components/eixo1/MaturidadeTripeUfChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Mapa da maturidade do tripé',
    component: MaturidadeTripeUfChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof MaturidadeTripeUfChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <MaturidadeTripeUfChart {...args} />
  </StoryFrame>
{/snippet}
