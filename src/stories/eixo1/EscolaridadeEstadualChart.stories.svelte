<script module lang="ts">
  ...
  import EscolaridadeEstadualChart from '$lib/components/eixo1/EscolaridadeEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Escolaridade dos gestores',
  ...
  type StoryArgs = ComponentProps<typeof EscolaridadeEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <EscolaridadeEstadualChart {...args} />
  </StoryFrame>
{/snippet}
