<script module lang="ts">
  ...
  import PatrimonioEstadualChart from '$lib/components/eixo1/PatrimonioEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Patrimônio cultural',
  ...
  type StoryArgs = ComponentProps<typeof PatrimonioEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <PatrimonioEstadualChart {...args} />
  </StoryFrame>
{/snippet}
