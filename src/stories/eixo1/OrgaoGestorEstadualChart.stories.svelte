<script module lang="ts">
  ...
  import OrgaoGestorEstadualChart from '$lib/components/eixo1/OrgaoGestorEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Órgão gestor',
  ...
  type StoryArgs = ComponentProps<typeof OrgaoGestorEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <OrgaoGestorEstadualChart {...args} />
  </StoryFrame>
{/snippet}
