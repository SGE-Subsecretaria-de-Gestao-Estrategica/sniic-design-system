<script module lang="ts">
  ...
  import CompetenciaConselhosEstadualChart from '$lib/components/eixo1/CompetenciaConselhosEstadualChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão estadual/Competência dos conselhos',
    component: CompetenciaConselhosEstadualChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof CompetenciaConselhosEstadualChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <CompetenciaConselhosEstadualChart {...args} />
  </StoryFrame>
{/snippet}
