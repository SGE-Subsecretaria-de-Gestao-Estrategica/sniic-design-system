<script module lang="ts">
  ...
  import ExecucaoLabMunicipalChart from '$lib/components/eixo1/ExecucaoLabMunicipalChart.svelte';
  import StoryFrame from '$lib/components/eixo1/StoryFrame.svelte';

  const { Story } = defineMeta({
    title: 'Cultura em Números/Eixo 1/Gestão e Participação/Gestão municipal/Execução da Lei Aldir Blanc',
    component: ExecucaoLabMunicipalChart,
    tags: ['autodocs'],
    parameters: { layout: 'padded' },
    args: { background: null },
  });

  type StoryArgs = ComponentProps<typeof ExecucaoLabMunicipalChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <ExecucaoLabMunicipalChart {...args} />
  </StoryFrame>
{/snippet}
