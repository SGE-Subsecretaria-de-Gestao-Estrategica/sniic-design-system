...
export default function useText(props: TextProps): {
  ...
  let { wordsWithWidth, spaceWidth } = $derived.by(() => {
    const words: string[] = text == null ? [] : text.toString().split(/(?:(?!\u00A0+)\s+)/);
    return {
  ...
  let wordsByLines = $derived.by(() => {
    ...
    return [
      {
        words: text == null ? [] : text.toString().split(/(?:(?!\u00A0+)\s+)/),
      },
    ];
  });

  let startDy = $derived.by(() => {
