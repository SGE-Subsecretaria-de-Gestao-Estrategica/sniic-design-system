    ...
    <svg {width} {height} role="img" aria-label={label}>
      ...
      {#if showCapitals && capitals.length}
        ...
        <g transform="translate({width - 120},{height - 100})">
          <text x={0} y={-8} font-size={10} font-family="'Rawline', 'Raleway', system-ui, sans-serif" fill="var(--chart-fg-strong, #000000)">Tamanho = valor recebido</text>
          {#each [0.25, 1] as frac, i (frac)}
            ...
            <text x={cx} y={br + 12} text-anchor="middle" font-size={10} font-family="'Rawline', 'Raleway', system-ui, sans-serif" fill="var(--chart-fg-strong, #000000)">
              {BRLFull.format(bval)}
            </text>
          {/each}
          <text x={0} y={52} font-size={10} font-family="'Rawline', 'Raleway', system-ui, sans-serif" fill="var(--chart-fg-strong, #000000)">Cor = execução (%)</text>
          {#each [['< 90%', '#d2301d'], ['~95%', '#ecb42d'], ['≥ 100%', '#4f8c4e']] as [lbl, clr], i (lbl)}
            <g transform="translate(0,{60 + i * 16})">
              <circle r={4} cx={4} fill={clr} opacity={0.85} />
              <text x={12} y={4} font-size={10} font-family="'Rawline', 'Raleway', system-ui, sans-serif" fill="var(--chart-fg-strong, #000000)">{lbl}</text>
            </g>
          {/each}
        </g>
      {/if}
