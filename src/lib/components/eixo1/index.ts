 ...
 * `sniic-design-system`. Charts sobre duas bases — `FaixaLinhasChart` (linhas)
 * e `ComposicaoChart` (colunas 100%) —, com paleta e tipografia próprias
 * (`cores.ts`, `tokens.ts`) — um sistema de charts autocontido, pensado para
 * exportação em SVG/PNG para impressão em A4, diferente do sistema interativo
 * (`ChartFrame`, `getPillarTheme`) usado pelo Eixo 6. Os demais componentes do
 * repo original (os que não usam essas duas bases) ainda não foram portados.
 */
